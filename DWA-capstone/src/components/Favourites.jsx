  ...
  const filteredAndSortedEpisodes = favoriteEpisodes
    ...
    .sort((a, b) => {
      ...
      if (sortType === "title-asc") {
        return getShowInfo(a.showID).title.localeCompare(
          getShowInfo(b.showID).title
        );
      }
      if (sortType === "title-desc") {
        return getShowInfo(b.showID).title.localeCompare(
          getShowInfo(a.showID).title
        );
      }
    ...
    <div>
      <Box>
        ...
        <ul className="no-list-style">
          {Array.isArray(filteredAndSortedEpisodes) &&
          filteredAndSortedEpisodes.length > 0 ? (
            filteredAndSortedEpisodes.map((episode) => {
              ...
              return (
                <li
                  className="fav--episodes"
                  key={`${episodeId}-${episode.showID}-${season}`}
                >
                  {image && <img className="fav--image" src={image} />}

                  <p>
                    Show: {title}, Season: {season}, Episode: {episodeId}, Last
                    updated: {new Date(updated).toLocaleString()}, Added on:{" "}
